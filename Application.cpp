...
void Application::createSwapchain() {
    ...
    for (auto& m : sc.presentModes)
        if (m == VK_PRESENT_MODE_MAILBOX_KHR) pm = m;
...
VulkanContext Application::makeContext() const {
    return {
        ...
        graphicsQueue_, graphicsFamily_,
        commandPool_, renderPass_, swapchainExtent_,
        getExeDir(),
    };
}
...
void Application::drawFrame() {
    ...
    for (int i = 0; i < (int)features_.size(); i++) {
        ...
        if (ImGui::Button(features_[i]->name())) activeFeature_ = i;
        if (active) ImGui::PopStyleColor();
        if (i + 1 < (int)features_.size()) ImGui::SameLine();
    }
