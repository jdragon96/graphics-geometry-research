...
void BucketVoxelHash::Initialize(VulkanContext &context)
{
    ...
    CreateTSDFRenderPipeline(context);
    CreateDeleteBoxShader(context);
    Clear(context);

    // kHashTableBuffer를 persistent map → queryTSDF에서 vkMapMemory 오버헤드 없이 읽기
    kHashTableBuffer.persistentMap<int32_t>();
}
...
void BucketVoxelHash::CreateTSDFRenderPipeline(VulkanContext &context)
{
    RenderingShader::PipelineOptions options{};
    options.topology = VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
    options.cullMode = VK_CULL_MODE_NONE;
    kTSDFRenderShader_.Initialize(
        context.device,
        context.renderPass,
        context.extent,
        context.basePath + "/shaders/VoxelHash_RenderTSDFPoint.vert.spv",
        context.basePath + "/shaders/VoxelHash_RenderPoint.frag.spv",
        {{0, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, VK_SHADER_STAGE_VERTEX_BIT}},
        options,
        sizeof(VHB_RenderPC));
    kTSDFRenderShader_.BindBuffer(0, kHashTableBuffer.GetBuffer());
}
...
void BucketVoxelHash::CreateUpdateTSDFShader(VulkanContext &context)
{
    ...
    kUpdateTSDFShader.BindBuffer(6, kSortedNormalBuffer.GetBuffer());
    kUpdateTSDFShader.BindBuffer(7, kSortedColorBuffer.GetBuffer());
...
// 셰이더 VoxelHash_UpdateHistogram.comp와 동일한 해시 함수
uint32_t BucketVoxelHash::hashBucket(int32_t kx, int32_t ky, int32_t kz)
{
    return (uint32_t(kx) * 73856093u
          ^ uint32_t(ky) * 19349663u
          ^ uint32_t(kz) * 83492791u) % VH_NUM_BUCKETS;
}

// VoxelHash_UpdateTSDF.comp의 packNormal() C++ 역함수
// octahedral encoding: uint32 (x_uint16 | y_uint16<<16) → unit vec3
Eigen::Vector3f BucketVoxelHash::decodeOctNormal(uint32_t packed)
{
    float x = (float(packed & 0xFFFFu) / 65535.f) * 2.f - 1.f;
    float y = (float(packed >> 16u)    / 65535.f) * 2.f - 1.f;
    float z = 1.f - (std::abs(x) + std::abs(y));  // L1 구 역투영
    if (z < 0.f)  // 하반구 역변환 (packNormal의 hemisphere fold 역)
    {
        float ox = x, oy = y;
        x = (1.f - std::abs(oy)) * (ox >= 0.f ? 1.f : -1.f);
        y = (1.f - std::abs(ox)) * (oy >= 0.f ? 1.f : -1.f);
    }
    Eigen::Vector3f n(x, y, z);
    float len = n.norm();
    return (len > 1e-6f) ? (n / len) : Eigen::Vector3f(0.f, 0.f, 1.f);
}

// ─────────────────────────────────────────────────────────────────────────────
//  queryTSDF — CPU O(1) 해시 조회
// ─────────────────────────────────────────────────────────────────────────────
TSDFQueryResult BucketVoxelHash::queryTSDF(float wx, float wy, float wz) const
{
    TSDFQueryResult result{0.f, Eigen::Vector3f::Zero(), 0.f, false};

    // persistent map 포인터 (Initialize()에서 한 번 매핑)
    const int32_t* raw = kHashTableBuffer.persistentMap<const int32_t>();
    if (!raw) return result;

    // 1. 월드 좌표 → 복셀 키
    const int32_t kx = static_cast<int32_t>(std::floor(wx / voxelSize_));
    const int32_t ky = static_cast<int32_t>(std::floor(wy / voxelSize_));
    const int32_t kz = static_cast<int32_t>(std::floor(wz / voxelSize_));

    // 2. 버킷 인덱스
    const uint32_t bucket = hashBucket(kx, ky, kz);

    // 3. 버킷 내 4개 슬롯 선형 탐색 (STRIDE = VH_ENTRY_INTS = 12)
    constexpr int STRIDE = VH_ENTRY_INTS;
    for (uint32_t slot = 0; slot < VH_BUCKET_SIZE; ++slot)
    {
        const int base = static_cast<int>((bucket * VH_BUCKET_SIZE + slot) * STRIDE);

        const int32_t entry_kx = raw[base + 0];
        if (entry_kx == VH_EMPTY_KEY) break;  // 빈 슬롯 → 이후 없음
        if (entry_kx != kx) continue;
        if (raw[base + 1] != ky) continue;
        if (raw[base + 2] != kz) continue;

        // 4. 매칭 → 필드 읽기 (float-as-int 역변환)
        float w;
        std::memcpy(&w, &raw[base + 4], sizeof(float));
        if (w <= 0.f) break;

        float tsdf;
        std::memcpy(&tsdf, &raw[base + 3], sizeof(float));

        result.tsdf   = tsdf;
        result.weight = w;
        result.normal = decodeOctNormal(static_cast<uint32_t>(raw[base + 6]));
        result.valid  = true;
        break;
    }
    return result;
...
// ─────────────────────────────────────────────────────────────────────────────
//  deleteBox — GPU box-wise clear
// ─────────────────────────────────────────────────────────────────────────────

void BucketVoxelHash::CreateDeleteBoxShader(VulkanContext &context)
{
    kDeleteBoxShader.Initialize(
        context.device,
        context.basePath + "/shaders/VoxelHash_DeleteBox.comp.spv",
        {{0}},
        sizeof(VH_DeleteBoxPC));
    kDeleteBoxShader.BindBuffer(0, kHashTableBuffer.GetBuffer());
}

void BucketVoxelHash::deleteBox(const Eigen::AlignedBox3f& box, VulkanContext& ctx)
{
    VH_DeleteBoxPC pc{};
    pc.minKx = static_cast<int32_t>(std::floor(box.min().x() / voxelSize_));
    pc.minKy = static_cast<int32_t>(std::floor(box.min().y() / voxelSize_));
    pc.minKz = static_cast<int32_t>(std::floor(box.min().z() / voxelSize_));
    pc._pad0 = 0;
    pc.maxKx = static_cast<int32_t>(std::floor(box.max().x() / voxelSize_));
    pc.maxKy = static_cast<int32_t>(std::floor(box.max().y() / voxelSize_));
    pc.maxKz = static_cast<int32_t>(std::floor(box.max().z() / voxelSize_));
    pc._pad1 = 0;

    // ResetHashTable()과 동일한 one-shot command buffer 패턴
    VkCommandBufferAllocateInfo ai{};
    ai.sType              = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    ...
    kDeleteBoxShader.Bind(cmd);
    kDeleteBoxShader.Push(cmd, pc);
    kDeleteBoxShader.Dispatch(cmd, (VH_TOTAL_ENTRIES + 63u) / 64u);
