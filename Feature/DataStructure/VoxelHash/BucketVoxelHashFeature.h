...
public:
    ...
    void onRender(const RenderContext &ctx) override
    {
        ...
        else if (renderMode_ == 1)
        {
            ...
            vhb_.RenderTSDF(ctx.commandBuffer, mvp.data());
    }
    ...
    void onImGui() override
    {
        ...
        ImGui::SameLine();
        if (ImGui::RadioButton("TSDF Point", renderMode_ == 2)) renderMode_ = 2;
        ImGui::Checkbox("Use Slot Point", &useSlotPoint_);
        if (renderMode_ == 1)
            ImGui::Checkbox("Show Voxel Slot Points", &showVoxelSlotPoint_);
        if (renderMode_ == 2)
            ImGui::Checkbox("TSDF Surface Projection", &tsdfProjectToSurface_);
        ImGui::Separator();
        ...
        cfgChanged |= ImGui::SliderFloat("Noise Stddev (m)", &cfg.noiseStddev, 0.0f, 0.05f, "%.4f");
        if (cfgChanged)
        {
...
private:
    ...
    void doIntegrate()
    {
        ...
        vhb_.setTSDFProjectToSurface(tsdfProjectToSurface_);
